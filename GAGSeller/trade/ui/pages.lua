...
return function(ctx)
	...
	------------------------------------------------------------------ SAM LOOP PAGE
	local samPage = makePage("Sam Loop", "Sam The Clam Loop", "🦪", 7)
	do
		local acc = makeAccordion(samPage, "Full Loop Sam The Clam", 1)
			...
			while ctx.alive() do
				...
				if ok and s then
					...
					samLbl.Text = string.format(
						...
						"Dicerna: <font color=\"#8c929e\">%s</font>\n" ..
						"Total Hop: <font color=\"#8c929e\">%s</font>\n" ..
						"Loop: <font color=\"%s\">%s</font>",
						col, s.state, col, s.timer, tostring(s.submitted or "-"),
						tostring(s.hops or 0),
						s.active and "#5acc78" or "#8c929e", s.active and "AKTIF" or "OFF")
				end
				task.wait(1)
			end
		end)

		-- Target menit (hop sampai timer <= ini)
		makeInput(acc, "Target Menit", "Hop sampai timer <= ini. 0 = sampai siap claim",
