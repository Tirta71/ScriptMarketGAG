...
local HOP_TTL  = 1800 -- detik; server dianggap "baru lagi" setelah 30 menit
local visited  = {}
do
...
local function queueResume()
	...
	if q then pcall(q, [[loadstring(readfile("GAGSniper.lua"))()]]) end
end
...
local function serverHop()
	...
	local function attempt()
		...
		local pool = getOpenServers(placeId)
		if #pool == 0 and not resetOnce then
			-- semua server sudah dikunjungi: reset daftar (kecuali server sekarang) lalu ambil ulang
			resetOnce = true
			setStatus("Semua server sudah dikunjungi, reset daftar...")
			visited = {}; markVisited(game.JobId)
			task.wait(1)
			pool = getOpenServers(placeId)
		end
		if #pool > 0 then
			local jobId = pool[1]           -- pool sudah diacak
			markVisited(jobId)              -- catat SEBELUM teleport supaya reload nggak balik ke sini
			setStatus(("Hop -> server baru (%d kandidat)"):format(#pool))
			if not pcall(function() TeleportService:TeleportToPlaceInstance(placeId, jobId, LP) end) then
				task.wait(1); attempt()
			end
		else
			-- benar-benar tidak ada kandidat (API rate-limit / server penuh semua): tunggu & coba lagi
			setStatus("Tidak ada server kandidat, tunggu lalu coba lagi...")
			task.wait(5); attempt()
		end
	end
	local conn
	conn = TeleportService.TeleportInitFailed:Connect(function(plr, _, msg)
		if plr == LP then log("Teleport gagal: " .. tostring(msg)); task.wait(1.5); if running then attempt() end end
	end)
	task.spawn(function()
		while hopInProgress and running do task.wait(12); if hopInProgress and running then log("Retry hop..."); attempt() end end
